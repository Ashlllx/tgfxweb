...
当 [<font color=blue> PAG 文件</font>](/docs/terms.html#pag-文件)中包含视频格式的BMP预合成时会需要视频解码器，在平台支持的情况下默认会优先选择硬解码器解码。目前在移动端上，能够支持动态注入用户自研的软件解码器。

### 如何接入
#### 1、include [<font color=blue>SoftwareDecoder.h</font>](https://pag.qq.com/file/SoftwareDecoder.h),派生实现如下2个父类：
...
### 注入解码器范例工程：
libpag 注入视频软件解码器的示例已经整合到我们的标准[<font color=blue>范例工程</font>](http://localhost:3000/docs/pad-demo-download.html)中。