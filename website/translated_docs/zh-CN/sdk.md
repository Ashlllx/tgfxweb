...
### SDK 获取
最新 SDK：[<font color=blue>SDK 下载页面</font>](https://github.com/tencent/libpag/releases)<br/>

其中企业版 SDK 包含 enterprise 字样，而社区版不包含该字样。<br/>

#### 社区版功能后缀:
- 无后缀：无后缀版本为 full 版本，Android 平台内部包含软件解码器，当硬解失败时回退使用，iOS 由于硬件解码器兼容性较好不内置软解。 
- noffavc： 后缀为 noffavc，版本内部不包含软件解码器，支持解码器外部注入，iOS 平台无该版本。

#### 企业版功能后缀:
- 无后缀：无后缀版本为基础版本，不包含 Movie 模块，不支持多字节 emoji，包含高阶 AE 特性。iOS 平台额外支持多字节 emoji 的能力。
- noffavc：后缀为 noffavc，不包含 Movie 模块和多字节 emoji 能力、内部不包含软件解码器，支持解码器外部注入。iOS 平台由于硬件解码器兼容性较好不内置软解无该版本。
- movie：后缀为 movie，包含音频播放、[<font color=blue>占位图</font>](/docs/terms.html#占位图)一键替换视频、导出视频文件和高阶 AE 特性以及多字节 emoji 的能力。

和社区版本不同的是，企业版本的接入需要 [<font color=blue>SDK 鉴权</font>](/docs/sdk-authorization.html)
...
[<font color=blue>官方 Demo 下载</font>](/docs/pad-demo-download.html)
...
#### Framework 接入<br>
   1. 将 libpag 的 framework 文件 libpag.xcframework 和 FFMovie.xcframework( movie 版本需要) 放置在 iOS 工程项目目录下。
   2. 在项目工程中，配置所使用 Target 的 General->Embedded Binaries，添加 libpag.xcframework 和 FFMovie.xcframework( movie 版本需要) 。
   3. 由于 Bitcode 苹果官方已废弃，配置 Build Settings->Build Options->Enable Bitcode 为 No

#### cocoapods 接入<br>
修改 App 目录下的 Podfile 文件，添加相应的 libpag 的引用.
...
**注意：** 最新 SDK 版本请参考：[<font color=blue>SDK 下载页面</font>](https://github.com/tencent/libpag/releases)<br/>

#### Swift Package Manager 接入
在 XCode 工程中打开 File > Swift Packages > Add Package Dependency
...
在工程目录下创建 carfile 文件，添加 libpag 依赖
```asm
  binary "https://libpag.github.io/pag-ios/Carthage/libpag.json" == 4.2.88
```
在工程目录下调用：
```asm
  Carthage update
...
加载成功后可以在 Carthage/Build 目录下找到 libpag.xcframework

**链接动态库** <br/>
打开工程，并将 libpag.xcframework 拖入工程设置的 General -> Frameworks, Libraries, and Embedded Content 中即可使用
...
**以上的兼容表仅代表可以运行的兼容性。对于有移动端接入需要的用户，需要阅读一下这篇 [<font color=blue>兼容性情况</font>](/docs/web-features.html#兼容性) 的文章**
...
点击「微信开发者工具」- 「工具」- 「构建 npm」，进行小程序 npm 依赖构建

2. 将 node_modules/libpag-miniprogram/lib/libpag.wasm.br 文件复制到 utils 目录下
3. 初始化 PAG
...
lite 版本：[<font color="blue">https://github.com/Tencent/libpag/tree/main/web/lite/wechat</font>](https://github.com/Tencent/libpag/tree/main/web/lite/wechat)
...
### 接入方式
桌面端（macOS、Windows、Linux）暂不提供统一的制品库，需要开发者根据源码自行构建，但我们会提供编译指引。PAG 是跨平台的，不同平台之间的 API 接口具有一致性。

具体原因如下：
1. Windows 端没有统一的 UI 框架；<br/>
2. Linux 端由于服务端 CPU 型号或系统版本的差异，统一的制品库难以完全兼容
...
### API 接口使用
在 PAG 的源码工程中，C++ 层基本上所有对外提供的方法都进行了测试用例的覆盖，如果想了解具体方法的使用，查看相关测试用例是一种很便捷的途径。
...
* `Visual Studio` 最低版本要求 VS2015pl
* `Node.js` PAG 依赖管理的基础
* `depsync` 用于 PAG 的依赖管理
...
2. 通过根目录下的 build_linux.sh 完成 pag 库文件的构建

3. 编译生成的头文件和库文件位于 linux/vendor 目录下

## 测试素材获取

[<font color=blue>PAG测试素材下载</font>](/docs/pag-test-material.html)<br/> 
