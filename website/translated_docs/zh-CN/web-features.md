...
libpag 核心代码为 C++ 代码，Web 平台上使用 WebAssembly 技术将 C++ 代码 编译成 libpag.wasm 文件运行于 Web 平台环境中。使用 libpag 可以不需要了解太多 WebAssembly 的知识， 如果有兴趣可以阅读 [<font color=blue>WebAssembly文档</font>](https://webassembly.org/) 和 [<font color=blue>emscripten文档</font>](https://emscripten.org/)。
...
因为 Firefox 的 Video 标签无法解析 PAG 动效文件中 BMP 序列帧转化而成的视频，所以需要注册软件解码器 ffavc 来解析。[<font color=blue>示例Demo</font>](https://github.com/libpag/pag-web/blob/main/pages/software-decoder.html)
...
以上两个环境兼容性的影响，可以使用注册软件解码器 ffavc 解析视频来规避，也是当前推荐的方案。[<font color=blue>示例Demo</font>](https://github.com/libpag/pag-web/blob/main/pages/software-decoder.html)
